...
for pathImg in pathImgs:
    ...
    LRimg = imgTransform(LRimgPIL).unsqueeze(0).to(device)
    
    scorecamMap, logits = scorecamVGG(img)
    LRscorecamMap, LRlogits = scorecamVGG(LRimg)
    ...
    visualize_multiscale(imgPIL, LRimgPIL, scorecamMap.cpu(), LRscorecamMap.cpu(), save_path)
